 ...
 */
package de.minigameslib.mclib.snakeyaml.composer;
...
public class ComposerImplTest extends TestCase {
    ...
    public void testComposeBean() {
        String data = "!!de.minigameslib.mclib.snakeyaml.composer.ComposerImplTest$BeanToCompose {name: Bill, age: 18}";
        Yaml yaml = new Yaml();
        Node node = yaml.compose(new StringReader(data));
        assertNotNull(node);
        assertTrue(node instanceof MappingNode);
        assertEquals(
                "tag:yaml.org,2002:de.minigameslib.mclib.snakeyaml.composer.ComposerImplTest$BeanToCompose",
