...
/**
 * Config option to edit int list variables.
 * 
 * @author mepeisen
 *
 */
public class IntListConfigOption extends AbstractConfigOption
{
    ...
    {
        final int[] prim = this.calculate(contextProvider, this.getValue()::getIntList);
        final Integer[] arr = new Integer[prim.length];
        ...
        player.nestClickGui(new TextListPage<>(
            ...
            str ->
            {
                try
                {
                    final int newVal = Integer.parseInt(str);
