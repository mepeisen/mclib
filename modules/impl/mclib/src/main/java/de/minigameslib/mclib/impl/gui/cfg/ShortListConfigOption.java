...
/**
 * Config option to edit short list variables.
 * 
 * @author mepeisen
 *
 */
public class ShortListConfigOption extends AbstractConfigOption
{
     ...
     *            value to be edited.
     */
    public ShortListConfigOption(ConfigurationValueInterface value)
    {
        super(value);
    }
    ...
    {
        final short[] prim = this.calculate(contextProvider, this.getValue()::getShortList);
        final Short[] arr = new Short[prim.length];
        ...
        player.nestClickGui(new TextListPage<>(
            ...
            a ->
            {
                this.run(contextProvider, () ->
                {
                    final short[] res = new short[a.size()];
                    for (int i = 0; i < res.length; i++)
                    {
                        res[i] = a.get(i);
                    }
                    getValue().setShortList(res);
                });
                onChange.run();
            },
            str ->
            {
                try
                {
                    final short newVal = Short.parseShort(str);
                    return (Short) newVal;
                }
