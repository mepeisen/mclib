...
/**
 * Config option to edit long variables.
 * 
 * @author mepeisen
 *
 */
public class LongConfigOption extends AbstractConfigOption
{
     ...
     *            value to be edited.
     */
    public LongConfigOption(ConfigurationValueInterface value)
    {
        super(value);
    }
    ...
    private void select(McPlayerInterface player, GuiSessionInterface session, ClickGuiInterface guiInterface, Runnable onChange, McRunnable contextProvider) throws McException
    {
        ...
        player.nestAnvilGui(new QueryText(
            ...
            s ->
            {
                try
                {
                    final long newVal = Long.parseLong(s);
                    this.run(contextProvider, () ->
                    {
                        this.getValue().setLong(newVal);
                        try
                        {
