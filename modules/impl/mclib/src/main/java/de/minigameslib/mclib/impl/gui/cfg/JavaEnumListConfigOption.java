 ...
 *
 */
public class JavaEnumListConfigOption extends AbstractConfigOption
{
     ...
     *            value to be edited.
     */
    public JavaEnumListConfigOption(ConfigurationValueInterface value)
    {
        super(value);
    }
    ...
    {
        final Class<? extends Enum<?>> clazz = this.getValue().getJavaEnumClass();
        final Enum<?>[] arr = this.calculate(contextProvider, () -> this.getValue().getJavaEnumList(clazz));
        player.nestClickGui(new JavaEnumListPage(
            ...
            a ->
            {
                this.run(contextProvider, () ->
                {
                    getValue().setJavaEnumList(a.toArray(new Enum<?>[a.size()]));
                    try
                    {
                        this.getValue().validate();
                        this.getValue().saveConfig();
