 ...
 *
 */
public class DummyHuman1_9 extends EntityPlayer
{
     ...
     * @param loc
     */
    public DummyHuman1_9(MinecraftServer minecraftserver, WorldServer worldserver, GameProfile gameprofile, PlayerInteractManager playerinteractmanager, Location loc)
    {
        ...
        this.updateTask = new BukkitRunnable() {
            ...
            public void run()
            {
                for (final UUID uuid : DummyHuman1_9.this.trackedPlayers)
                {
                    ...
                    if (player != null)
                    {
                        ...
                        if (DummyHuman1_9.this.respawn)
                        {
                            // if forced resapwn (teleport etc.) and it is no more in range we delete the entity so that it disappears
                            if (!newInRange)
                            {
                                sendPackages(con, 1,
                                    new PacketPlayOutEntityDestroy(DummyHuman1_9.this.getId()));
                            }
                            // override inrange to false so that players still in range will be forced to get a clean respawn
                            DummyHuman1_9.this.inRange.put(uuid, Boolean.FALSE);
                        }
                        
                        boolean oldInRange = DummyHuman1_9.this.inRange.get(uuid);
                        if (newInRange != oldInRange)
                        {
                            if (!oldInRange)
                            {
                                final byte encodedyaw = toAngle(DummyHuman1_9.this.yaw);
                                float body = DummyHuman1_9.this.yaw + 45;
                                if (body >= 180)
                                    body -= 360;
                                final byte encodedbody = toAngle(body);
                                sendPackages(con, 1,
                                    new PacketPlayOutEntityDestroy(DummyHuman1_9.this.getId()),
                                    new PacketPlayOutPlayerInfo(EnumPlayerInfoAction.ADD_PLAYER, DummyHuman1_9.this),
                                    new PacketPlayOutNamedEntitySpawn(DummyHuman1_9.this));
                                sendPackages(con, 2,
                                    new PacketPlayOutEntityHeadRotation(DummyHuman1_9.this, encodedyaw),
                                    new PacketPlayOutEntity.PacketPlayOutEntityLook(DummyHuman1_9.this.getId(), encodedbody, toAngle(DummyHuman1_9.this.pitch), true));
                                sendPackages(con, 4,
                                    new PacketPlayOutPlayerInfo(EnumPlayerInfoAction.REMOVE_PLAYER, DummyHuman1_9.this));
                            }
                            DummyHuman1_9.this.inRange.put(uuid, newInRange);
                        }
                    }
                }
                DummyHuman1_9.this.respawn = false;
            }
        };
    ...
    public void untrack(Player player)
    {
        new BukkitRunnable() {
            ...
            public void run()
            {
                DummyHuman1_9.this.trackedPlayers.remove(player.getUniqueId());
                DummyHuman1_9.this.inRange.remove(player.getUniqueId());
            }
    ...
    public void track(Player player)
    {
        new BukkitRunnable() {
            ...
            public void run()
            {
                DummyHuman1_9.this.trackedPlayers.add(player.getUniqueId());
                DummyHuman1_9.this.inRange.put(player.getUniqueId(), Boolean.FALSE);
                final PlayerConnection con = ((CraftPlayer) player).getHandle().playerConnection;
                con.sendPacket(new PacketPlayOutPlayerInfo(EnumPlayerInfoAction.REMOVE_PLAYER, DummyHuman1_9.this));
            }
        }.runTaskLater((Plugin) McLibInterface.instance(), 1);
    }
    ...
    protected static final class HumanNPC extends CraftPlayer
    {
         ...
         * @param handle
         */
        public HumanNPC(DummyHuman1_9 handle)
        {
