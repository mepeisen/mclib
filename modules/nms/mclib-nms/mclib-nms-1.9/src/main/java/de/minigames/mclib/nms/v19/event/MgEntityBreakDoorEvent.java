...
import de.minigameslib.mclib.api.event.McEntityBreakDoorEvent;
...
public class MgEntityBreakDoorEvent extends AbstractMinigameEvent<EntityBreakDoorEvent, McEntityBreakDoorEvent> implements McEntityBreakDoorEvent
{
     ...
     * @return player
     */
    private static McPlayerInterface player(EntityBreakDoorEvent event)
    {
     ...
     * @return arena
     */
    private static ZoneInterface location(EntityBreakDoorEvent event)
    {
        final Entity passenger = event.getEntity();
        ...
        return ObjectServiceInterface.instance().findZone(event.getBlock().getLocation());
    }
