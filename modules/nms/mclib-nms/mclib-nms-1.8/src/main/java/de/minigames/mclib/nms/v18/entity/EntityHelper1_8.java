...
public class EntityHelper1_8 implements EntityHelperInterface
{
    ...
    /** dummy humans. */
    private static final Set<DummyHuman1_8> HUMANS = new HashSet<>();
    ...
    private final LoadingCache<UUID, PropertyMap> properties = CacheBuilder.newBuilder().
            ...
            build(new CacheLoader<UUID, PropertyMap>(){
                ...
                public PropertyMap load(UUID key) throws Exception
                {
                    ...
                    return EntityHelper1_8.this.getRemoteProfile(player);
                }
            });
        ...
        Bukkit.getScheduler().scheduleSyncDelayedTask((Plugin) McLibInterface.instance(), new Runnable() {
            ...
            public void run()
            {
                final PacketPlayOutPlayerInfo packet = new PacketPlayOutPlayerInfo(
                        EnumPlayerInfoAction.REMOVE_PLAYER, new EntityPlayer[] { result });
...
//        });
    ...
    {
        for (final DummyHuman1_8 human : HUMANS)
        {
            final PacketPlayOutPlayerInfo packet = new PacketPlayOutPlayerInfo(
                    EnumPlayerInfoAction.REMOVE_PLAYER, new EntityPlayer[] { human });
