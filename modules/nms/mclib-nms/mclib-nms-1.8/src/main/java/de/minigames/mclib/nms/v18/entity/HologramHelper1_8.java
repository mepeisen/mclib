...
public class HologramHelper1_8 implements HologramHelperInterface
{
        ...
        protected void sendInRangePackages(Player player, PlayerConnection con)
        {
            ...
            for (int i = 0; i < this.lines.size(); i++)
            {
                ...
                if (text.length > i)
                {
                    ...
                    try
                    {
                        final List<WatchableObject> items = (List<WatchableObject>) PacketMetadataItemsField.get(meta);
                        for (int j = 0; j < items.size(); j++)
                        {
                            if (items.get(j).a() == EntityNameField)
                            {
                                items.set(j, new WatchableObject(EntityNameField, EntityNameDataType, text[i]));
                                break;
                            }
                        }
                    }
