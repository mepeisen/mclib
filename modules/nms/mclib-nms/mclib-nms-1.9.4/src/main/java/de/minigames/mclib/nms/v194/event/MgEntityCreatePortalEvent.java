...
import de.minigameslib.mclib.api.event.McEntityCreatePortalEvent;
...
public class MgEntityCreatePortalEvent extends AbstractMinigameEvent<EntityCreatePortalEvent, McEntityCreatePortalEvent> implements McEntityCreatePortalEvent
{
     ...
     * @return player
     */
    private static McPlayerInterface player(EntityCreatePortalEvent event)
    {
     ...
     * @return arena
     */
    private static ZoneInterface location(EntityCreatePortalEvent event)
    {
        final Entity passenger = event.getEntity();
        ...
        return ObjectServiceInterface.instance().findZone(event.getBlocks().get(0).getLocation()); // TODO correct call to get(0)?
    }
