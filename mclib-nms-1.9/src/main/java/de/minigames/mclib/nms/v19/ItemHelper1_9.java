...
import de.minigames.mclib.nms.v19.blocks.CustomDropper;
...
public class ItemHelper1_9 implements ItemHelperInterface
{
    ...
    public void setBlockVariant(Block block, int type, int variant)
    {
        ...
        final net.minecraft.server.v1_9_R1.Block nms = net.minecraft.server.v1_9_R1.Block.getById(type);
        @SuppressWarnings("deprecation")
    ...
    public Map<String, Integer> getBlockMappings()
    {
        ...
        for (final MinecraftKey key : net.minecraft.server.v1_9_R1.Block.REGISTRY.keySet())
        {
            final String name = key.toString();
            for (int i = 0; i < 16; i++)
            {
                final net.minecraft.server.v1_9_R1.Block block = net.minecraft.server.v1_9_R1.Block.REGISTRY.get(key);
                final int numid = net.minecraft.server.v1_9_R1.Block.REGISTRY.a(block);
    ...
    public ChunkDataImpl getChunkSnapshot(Chunk chunk)
    {
        ...
        final net.minecraft.server.v1_9_R1.Chunk handle = ((CraftChunk)chunk).getHandle();
        ...
        for (int x = 0; x < 16; x++)
        {
            ...
            for (int y = 0; y < 256; y++)
            {
                ...
                for (int z = 0; z < 16; z++)
                {
                    ...
                    blockData[x][y][z] = net.minecraft.server.v1_9_R1.Block.REGISTRY_ID.getId(data);
                }
            }
        }
    ...
    public void loadTileEntities(Location loc, List<TileEntityData> data)
    {
        for (final TileEntityData tiledata : data)
        {
            ...
            ((CraftChunk)newLoc.getChunk()).getHandle().tileEntities.put(
                ...
                TileEntity.a(((CraftWorld)loc.getWorld()).getHandle().getMinecraftServer(), tag));
        }
    }
