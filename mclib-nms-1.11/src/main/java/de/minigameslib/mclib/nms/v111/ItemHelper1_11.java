...
import de.minigameslib.mclib.nms.v111.blocks.CustomDropper;
import de.minigameslib.mclib.nms.v111.blocks.CustomHopper;
import de.minigameslib.mclib.nms.v111.items.ConfigItemStackDataImpl;
import de.minigameslib.mclib.nms.v111.items.CustomArmor;
import de.minigameslib.mclib.nms.v111.items.CustomAxe;
import de.minigameslib.mclib.nms.v111.items.CustomHoe;
import de.minigameslib.mclib.nms.v111.items.CustomItem;
import de.minigameslib.mclib.nms.v111.items.CustomPickaxe;
import de.minigameslib.mclib.nms.v111.items.CustomShovel;
import de.minigameslib.mclib.nms.v111.items.CustomSword;
...
public class ItemHelper1_11 implements ItemHelperInterface
{
    ...
    /** Logger. */
    private static final Logger LOGGER = Logger.getLogger(ItemHelper1_11.class.getName());
    ...
    public void setBlockVariant(Block block, int type, int variant)
    {
        ...
        final net.minecraft.server.v1_11_R1.Block nms = net.minecraft.server.v1_11_R1.Block.getById(type);
        @SuppressWarnings("deprecation")
    ...
    {
        final ItemMeta meta = ItemHelper1_11.getMeta(stack);
    ...
    {
        RecipesFurnace.getInstance().a(InventoryManager1_11.convertToNms(new ItemStack(itemId, 1)), InventoryManager1_11.convertToNms(receipe), experience);
    }
     ...
     */
    private void replaceModdedBlock(int numBlockId, net.minecraft.server.v1_11_R1.Block block)
    {
        try
        {
            final Field bMapField = net.minecraft.server.v1_11_R1.RegistryMaterials.class.getDeclaredField("b"); //$NON-NLS-1$
            bMapField.setAccessible(true);
            @SuppressWarnings("unchecked")
            final Map<net.minecraft.server.v1_11_R1.Block, MinecraftKey> bMap = (Map<net.minecraft.server.v1_11_R1.Block, MinecraftKey>) bMapField.get(net.minecraft.server.v1_11_R1.Block.REGISTRY);
            
            final Field aMapField = net.minecraft.server.v1_11_R1.RegistryMaterials.class.getDeclaredField("a"); //$NON-NLS-1$
            aMapField.setAccessible(true);
            @SuppressWarnings("unchecked")
            final RegistryID<net.minecraft.server.v1_11_R1.Block> aMap = (RegistryID<net.minecraft.server.v1_11_R1.Block>) aMapField.get(net.minecraft.server.v1_11_R1.Block.REGISTRY);
            
            bMap.put(block, bMap.remove(net.minecraft.server.v1_11_R1.Block.getById(numBlockId)));
    ...
    {
        final CustomBlock block = (CustomBlock) net.minecraft.server.v1_11_R1.Block.getById(blockId);
    ...
    public Map<String, Integer> getBlockMappings()
    {
        ...
        for (final MinecraftKey key : net.minecraft.server.v1_11_R1.Block.REGISTRY.keySet())
        {
            final String name = key.toString();
            for (int i = 0; i < 16; i++)
            {
                final net.minecraft.server.v1_11_R1.Block block = net.minecraft.server.v1_11_R1.Block.REGISTRY.get(key);
                final int numid = net.minecraft.server.v1_11_R1.Block.REGISTRY.a(block);
    ...
    public ChunkDataImpl getChunkSnapshot(Chunk chunk)
    {
        ...
        final net.minecraft.server.v1_11_R1.Chunk handle = ((CraftChunk)chunk).getHandle();
        ...
        for (int x = 0; x < 16; x++)
        {
            ...
            for (int y = 0; y < 256; y++)
            {
                ...
                for (int z = 0; z < 16; z++)
                {
                    ...
                    blockData[x][y][z] = net.minecraft.server.v1_11_R1.Block.REGISTRY_ID.getId(data);
                }
            }
        }
